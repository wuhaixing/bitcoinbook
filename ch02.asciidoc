...
*这是不是说只要知道一个人的所有地址，就能知道TA有多少钱？这让人还怎么哭穷？！！！*
...
*一条交易的输出中含有一个script，如果把交易看做一个装有一定额度比特币的盒子，那么这个script就像一把锁一样，只有解开这个script才能用这条交易中的比特币。用signature可以解开这个script，而只有跟收款方的公开账号地址相匹配的key才能产生这样的signature，也就是说只有收款方才能解开这个script。这个script是怎么产生的呢？钱包又是如何判断拿到的signature能把它解开的呢？*
